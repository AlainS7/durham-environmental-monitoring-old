...
Usage:
  ...
  python scripts/test_bigquery_upload.py --dry-run

  # Real run (requires Google Application Default Credentials and REAL_BQ=1)
  REAL_BQ=1 python scripts/test_bigquery_upload.py --dataset your_dataset --table test_upload
...
log = logging.getLogger("test_bigquery_upload")
...
def upload_to_bigquery(df: pd.DataFrame, dataset: str, table: str, dry_run: bool = True):
    ...
    # Create dataset if not exists (conservative)
    try:
